...
\caption{Layered system architecture with strict separation of concerns: End-user channels $\rightarrow$ API Gateway $\rightarrow$ Agentic Backend (Trip Orchestrator + 4 specialized agents) $\rightarrow$ Domain Services (Itinerary Optimizer, Routing \& Transport, Climate \& Seasonality, Events \& Festivals, Cultural Knowledge, Alerts Scraper, Notification Service) $\rightarrow$ Shared Memory (PostgreSQL, Qdrant, Neo4j, Redis) $\rightarrow$ External APIs (Weather, Maps, Transport, Tourism, News).}
\label{fig:arch}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Hotel Owner:} Manages accommodation inventory and bookings
    \item \textbf{Restaurant Owner:} Manages dining reservations and offerings
    \item \textbf{Activity Owner:} Manages activity inventory and participant slots
    \item \textbf{System:} Monitors external feeds, detects disruptions proactively
    \item \textbf{Admin:} Operates services, manages configuration, reviews metrics and disputes
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{usecase-diagram.png}
\caption{System use cases for 5 stakeholders: Traveler (10 UCs: create trip, view timeline, preview details, report disruption, ask cultural questions, update preferences, book accommodation, dining, activities, receive notifications); Hotel Owner (7 UCs: register, create availability, manage bookings, view details, update pricing, handle cancellations, view reports); Restaurant Owner (7 UCs: register, create slots, manage reservations, view details, update menu, handle cancellations, view reports); Activity Owner (7 UCs: register, create slots, manage bookings, view participant details, update pricing, handle cancellations, view reports); Admin (7 UCs: manage users, monitor health, manage providers, review analytics, configure settings, handle disputes, view logs).}
\label{fig:usecase}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{trip_planning_erd.png}
\caption{Database entity-relationship model: User (id, email, phone, preferences) $\rightarrow$ 1:many $\rightarrow$ Trip (id, dates, destination, status); Trip $\rightarrow$ 1:many $\rightarrow$ Timeline Node (id, type, times, location, state, details, cultural context, risk indicators); Timeline Node $\rightarrow$ 1:many $\rightarrow$ Booking (id, provider, confirmation, status); Trip $\rightarrow$ 1:many $\rightarrow$ Alert (id, type, severity, timestamps); Trip $\rightarrow$ 1:many $\rightarrow$ Agent Run (id, agent, input/output, trace).}
\label{fig:erd}
\end{figure}
