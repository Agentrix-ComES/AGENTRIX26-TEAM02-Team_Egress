...
% fancyhdr requires more vertical room for the running header
\setlength{\headheight}{14pt}
\setlength{\headsep}{16pt}

% Map common Unicode characters used in the source so pdfLaTeX accepts them
...
% Look for figures in the current dir and in the sibling latex/ folder
\graphicspath{{./}{../latex/}}

% Safe include: if the image file is missing, draw a placeholder box instead
% so the document still compiles. Usage: \safeincludegraphics[opts]{file}
\newcommand{\safeincludegraphics}[2][width=0.95\textwidth]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \fbox{\parbox{0.9\textwidth}{\centering\ttfamily\small
      [missing image: #2]\\[2pt]
      Place the file in \texttt{docs/latex2/} (or \texttt{docs/latex/}) and recompile.}}%
  }%
}
...
\caption{Layered system architecture: end-user channels $\rightarrow$ API gateway $\rightarrow$ agentic backend $\rightarrow$ domain services $\rightarrow$ shared memory $\rightarrow$ external APIs.}
\label{fig:arch}
\end{figure}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{usecase-diagram.png}
\caption{Use cases across five actors: Traveler, Hotel Owner, Restaurant Owner, Activity Owner, and Admin.}
\label{fig:usecase}
\end{figure}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{trip_planning_erd.png}
\caption{Entity-relationship model: User $\rightarrow$ Trip $\rightarrow$ Timeline Node $\rightarrow$ Booking, with Alerts and Agent Runs attached to each Trip.}
\label{fig:erd}
\end{figure}
